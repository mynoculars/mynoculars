"""
logging_setup.py — ONE instrumentation path, TWO presentations.

Purpose:
    log_event() is the only function any module in this codebase calls to
    record anything. Everything else in this file is presentation: two
    logging.Handler/Formatter pairs subscribed to that same stream of
    LogRecords, producing two different views of the identical events —

      JsonLineFormatter    machine-parseable JSON-lines, always on, one
                           line per event, unchanged in shape from before
                           this file grew a second view.
      NarrativeFormatter + a human-readable execution story, one file per
      NarrativeBufferHandler  run (logs/run-<run_id>.txt), written only
                           when narrative capture is enabled (--debug /
                           DEBUG_TRACE=true — see tracing.py::Tracer,
                           which is now just the thing that turns this
                           handler on and flushes it, not a second
                           recorder with its own file format).

    Before this design, tracing.py's Tracer had its OWN record_llm()/
    record_retrieval() methods, called SEPARATELY from (and in addition
    to) the log_event() call already describing the same LLM/retrieval
    call — two independent instrumentation paths for one event, the exact
    thing this design collapses into one. See llm/client.py's complete()
    and storage/{qdrant_store,opensearch_store}.py's search() for the
    call sites that used to call both and now call only log_event().
...
Responsibilities:
    ...
    - Provide the narrative handler two functions turn on/flush: neither
      is a second logging framework — both operate on LogRecords the SAME
      log_event() calls already produced.
...
Design decision (stdlib over structlog):
    ...
    including "one event, two renderings," which is exactly what stdlib
    logging's handler/formatter split exists for.
