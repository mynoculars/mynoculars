...
# D-157: None when this runs from an installed package rather than a
# checkout, in which case `--golden` is required rather than defaulted.
# A deployment's golden set is ITS queries against ITS corpus; this
# repository's eight are calibration for its own ten Redis documents and
# are not a sensible default for anyone else's data.
DEFAULT_GOLDEN = repo_file("sample_data", "golden_queries.jsonl")
...
def main(argv=None) -> int:
    ...
    p.add_argument("--golden",
                   default=str(DEFAULT_GOLDEN) if DEFAULT_GOLDEN else None,
                   required=DEFAULT_GOLDEN is None,
                   help="golden-set JSONL (default "
                        + (DEFAULT_GOLDEN.name if DEFAULT_GOLDEN
                           else "required: no checkout to default to")
                        + ")")
