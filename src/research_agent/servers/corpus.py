...
def main() -> int:
    """Run the server until interrupted. Returns an exit code (D-157).

    The `if __name__ == "__main__"` block below used to call
    `mcp.run(...)` directly, which is fine for `python <file>` and gives a
    console entry point nothing to call. Host and port are already parsed
    at IMPORT time -- they are FastMCP CONSTRUCTOR arguments, see
    _parse_args above -- so this function deliberately takes no argv: by
    the time it runs, the binding is already decided.
    """
