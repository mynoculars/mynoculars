...
def route_after_goals(state: ResearchState) -> Literal["task_expander", "compiler"]:
    """D-21: zero goals -> straight to the compiler for an error report."""
    return "compiler" if state.planning_error else "task_expander"


def dispatch_tasks(state: ResearchState) -> Union[List[Send], Literal["compiler"]]:
...
def route_convergence(state: ResearchState, settings: Settings
                      ) -> Literal["compiler", "gap_generator"]:
...
def build_graph(router: FallbackRouter, tool: ToolFn, memory: SemanticMemory,
    ...
    g.add_node("goal_manager", build_goal_manager_node(router))
