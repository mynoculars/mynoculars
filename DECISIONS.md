...
| **D-26** | Retrieval tools are mediated through a plain callable seam today (`ToolFn`) — the original design's MCP stand-in (D-30) is now the SAME seam, not a replacement of it: `tools/corpus_search.py` and `tools/mcp_client.py` are both `ToolFn` implementations, chosen (and, since P2-14, combined) at `cli.py`'s wiring layer. Implemented, P2-13. | `tools/corpus_search.py`, `tools/mcp_client.py` |
