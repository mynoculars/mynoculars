...
| **D-26** | Retrieval tools are mediated through a plain callable seam today (`ToolFn`), a deliberate stand-in for MCP (D-30) — the graph-level pattern is identical either way. Implementation is P2-13 (not yet built). | `tools/corpus_search.py` |
| **D-27** | Memory retrieval fusion + decay is designed to run server-side in Qdrant (nested RRF + `FormulaQuery` decay, mandatory payload indexes) — **design-only**; this core build does it in Python (`memory/semantic_memory.py::decay_factor`, kept as the parity oracle for the eventual server-side version). Implementation is P2-10. | `design/Research_Agent_Design.md` §10 |
...
| **D-30** | MCP transport policy (stdio local / Streamable HTTP remote; SSE prohibited) plus tool-security invariants (join-then-resolve sandboxed paths, explicit per-server env allowlist — never forward `os.environ`, `AsyncExitStack` lifecycle). **Design-only** until P2-13. | `design/Research_Agent_Design.md` §10 |
