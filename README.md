...
> this revision (`DECISIONS.md` D-1…D-183) -- correctness fixes,
...
| `llm_retry_after_max_seconds` *(D-183)* | `60` | cap on how long the router will wait out a 429's OWN named cooldown (its `Retry-After` header, or, failing that, its body) before retrying that SAME provider once instead of hopping. A signal larger than this is not clamped and retried — it hops immediately, same as a 429 that names no duration at all (Mistral's observed shape). `0` disables the feature and restores the pre-D-183 behaviour: every 429 hops like any other error |
...
**D-183, and it applies identically at every hop shown above.** A "transport
error" at hop 0 or hop 1 that is specifically a 429 is no longer an automatic
hop. If that response names its own cooldown — a `Retry-After` header, or
(Google's shape) a `retryDelay`/prose duration in the body — the router waits
that long (capped at `LLM_RETRY_AFTER_MAX_SECONDS`, default 60s) and retries
the **same** provider once before falling through to the next one. Live
(p205.315/.316/.317-check) Gemini's own reported cooldown was 0.059s, then
35.36s, then 8.98s across three separate runs against the same free-tier
per-minute limit — every one of those comfortably worth waiting out instead of
immediately burning the rest of the chain. Mistral's 429 in the same session
named no duration anywhere and was never retried; `0` disables the whole
feature and restores the exact pre-D-183 diagram above. A second consecutive
429 on the same provider, even one that names another duration, is **not**
waited on again — it hops like any other failure.
...
| `cited_figures_misattributed` / `misattributed_figures` *(D-179)* | a figure the run DID retrieve, cited to a goal whose evidence does not carry it. A citation defect, not an invented number, and the two have **opposite remedies** — delete the figure, versus cite the goal that supports it. Collapsed into one finding, this made the system destroy correct content: live (`p205.308-check`) an evidence item read *"a deployed force of 975,000 troops"* under `g3`, the report cited `[g1]`, the verification judge — shown only `g1`'s evidence and asked whether the evidence supports it *in any form* — confirmed it as unsupported, and the rewrite deleted a true figure and asserted *"No retrieved evidence supports the specific figure of 975,000 troops"*. Deliberately does NOT trip `CAP_UNSUPPORTED_FIGURES`. **D-193:** the `misattributed_figures` sample was structurally empty until then — `_audit_figures` filtered its findings list to build the unsupported-only WARNING and returned the filtered list, so the sample searched a list every entry had already been removed from. On a run record predating D-193, a nonzero count beside `[]` here is that bug, not a clean report. | `guardrails/claims.py` |
| `cited_figures_recalled` / `recalled_figures` *(D-192)* | a figure the compiler took from **recalled memory** rather than from evidence this run retrieved. P2-02 files memory hits under `memory::<an earlier run's goal id>`, which is deliberately uncitable — the namespace exists so an old run's `g3` can never satisfy this run's `g3` by string collision. So this is neither of the two kinds above: not invented (the text was in the compile prompt — `prompts/budget.py` budgets memory items in), and not misfiled (nothing citable holds it). Before this it was reported as `misattributed` with `supported_by: ["memory::g3"]` — a remedy naming a goal no sentence can cite, i.e. unactionable by construction; live (`p205.333-check`) three of four findings had that shape. Like `misattributed` it does NOT trip `CAP_UNSUPPORTED_FIGURES`; `report.recalled_cited_figures` fires at WARNING, because a claim resting on a previous run is not grounded. `recalled_figures` inherited D-179's empty-sample bug on the day it was added and was fixed by D-193 | `guardrails/claims.py` |
| `figures_in_disclaimed_sentences` *(D-192)* | figures stated inside a sentence that asserts the **absence** of support — *"However, no document specifies whether current spending (~4% of GDP) is explicitly cited"*. Auditing such a sentence for support is backwards: the sentence and the audit agree, and reporting it as unsupported penalises the compiler for the hedging D-51 asks of it. Live, twice (`p205.330-check`, `p205.333-check`), a report was capped at `LOW (40%)` by `CAP_UNSUPPORTED_FIGURES` for exactly this. Excluded from `cited_figures_checked`, and **counted** rather than silently dropped, on D-174's rule that an exclusion nothing can see is a loophole. Deliberately narrow: it matches a statement about *documents, sources, evidence or confirmation*, never the bare word "not" — *"the PLAN does not field 300 major surface combatants"* is a claim about the world and stays audited | `guardrails/claims.py` |
| `critique_notes_dismissed` *(D-155, D-182)* | how many critic notes were PROVEN FALSE this cycle and pruned before the next compile: a note disputing a figure the evidence actually contains (D-155), or claiming a report sentence lacks a citation tag it demonstrably carries (D-182, checked against `authored` -- the models own text, D-139). Only emitted on a FAILED critique with at least one such note; a clean pass never sets it. **Nonzero with `critique_passed: false` is new as of D-182** -- before, a note this cannot adjudicate (coverage, semantics) left every note standing, proven-false ones included, so a partially-disprovable critique sent all of it to the next compile as things to fix. `critic.notes_partially_dismissed` (WARNING) records that case; `critic.failure_not_corroborated` (WARNING) still covers the case where nothing survives and the verdict flips to `true`. Live (`p205.309-check`) 18 of 21 notes in one critique pass quoted a sentence that DID carry the `[gN]` marker the note said was missing | `guardrails/critique.py`, `agents/compilation.py` |
...
authoritative consolidated log, currently D-1 through D-183 — this table is a
